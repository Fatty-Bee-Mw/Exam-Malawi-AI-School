...
export default function Navbar() {
  const { currentUser, logout, applySubscription } = useAuth();
  ...
  const handleSubscribe = (_planId, _paymentId, subscription) => {
    if (subscription) {
  ...
  return (
    <>
      <nav className="flex-shrink-0 bg-dark-secondary border-b border-dark-muted z-50">
          ...
          <div className="flex items-center justify-between h-12">
            ...
            <div className="hidden md:block">
              <div className="ml-10 flex items-baseline space-x-4">
                <Link to="/dashboard" className="text-gray-300 hover:text-dark-neon-blue px-3 py-2 rounded-md text-sm font-medium transition-colors">
                  <HomeIcon className="h-4 w-4 inline mr-2" />
                  Dashboard
                </Link>
                {isAdmin && (
                  <Link to="/admin" className="text-gray-300 hover:text-dark-neon-purple px-3 py-2 rounded-md text-sm font-medium transition-colors">
                ...
                <div className="flex items-center space-x-2 ml-4 pl-4 border-l border-dark-muted">
                  <span className={`px-2 py-1 text-xs rounded-full ${
                    ...
                    {currentPlan === 'premium' ? 'PREMIUM' : currentUser?.subscription?.active ? currentUser.subscription.plan_id?.toUpperCase() : 'FREE'}
                  </span>
        ...
        {isMenuOpen && (
          <div className="md:hidden">
            ...
            <Link to="/dashboard" className="text-gray-300 hover:text-dark-neon-blue block px-3 py-2 rounded-md text-base font-medium">
              Dashboard
            </Link>
            {isAdmin && (
              <Link to="/admin" className="text-gray-300 hover:text-dark-neon-purple block px-3 py-2 rounded-md text-base font-medium">
                Admin Panel
              </Link>
            )}
